...
static SDL_Texture* create_button_texture( int hpkey, bool is_up )
{
    ...
    } else if ( BUTTONS[ hpkey ].label_bitmap != ( unsigned char* )0 ) {
        /* Button has a texture */
        x = ( 1 + BUTTONS[ hpkey ].w - BUTTONS[ hpkey ].label_bitmap_w ) / 2;
        y = ( 1 + BUTTONS[ hpkey ].h - BUTTONS[ hpkey ].label_bitmap_h ) / 2;
        if ( is_up )
            y += 1;

        __draw_bitmap( x, y, BUTTONS[ hpkey ].label_bitmap_w, BUTTONS[ hpkey ].label_bitmap_h, BUTTONS[ hpkey ].label_bitmap,
