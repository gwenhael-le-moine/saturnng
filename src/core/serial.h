...
/*---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------*/
typedef enum {
    SERIAL_I_CALLED = 101,      /* Function %s called */
    SERIAL_I_REVISION = 102,    /* Serial port emulation rev. %s */
    SERIAL_I_READ = 103,        /* Read %s -> %x */
    SERIAL_I_WRITE = 104,       /* Write %s %x -> %x */
    SERIAL_I_RBR = 105,         /* Read RBR -> %x */
    SERIAL_I_TBR = 106,         /* Write TBR <- %x */
    SERIAL_I_PTY_NAME = 107,    /* pty name is %s */
    SERIAL_W_EMPTY_RRB = 201,   /* Read from empty RX buffer, rcs=%x */
    SERIAL_W_FULL_TRB = 202,    /* Write into full TX buffer, tcs=%x */
    SERIAL_W_NOPTY = 203,       /* 3.16: Pty support not available */
    SERIAL_E_TRB_DRAIN = 301,   /* Error draining TX buffer */
    SERIAL_E_RRB_CHARGE = 302,  /* Error charging RX buffer */
    SERIAL_E_PTY_CLOSE = 303,   /* Error closing pty */
    SERIAL_F_OPENPTY = 401,     /* openpty() failed on master pty */
    SERIAL_F_FCNTL = 402,       /* fcntl() failed on master pty */
    SERIAL_F_OPEN_MASTER = 403, /* Can't open pty master %s */
    SERIAL_F_GRANTPT = 404,     /* grantpt() failed on master pty */
    SERIAL_F_UNLOCKPT = 405,    /* unlockpt() failed on master pty */
    SERIAL_F_OPEN_SLAVE = 406,  /* Can't open pty slave %s */
