...
{
    for ( int i = 0; i < m; i++ )
        strcat( ob, hex_digit[ ( int )bus_fetch_nibble( start + m - i - 1 ) ] );
...
{
    sprintf( ob, "P=%d", bus_fetch_nibble( pc ) );
    pc++;
    return pc;
}
...
{
    Nibble m = bus_fetch_nibble( pc ) + 1;
    pc++;
...
{
    Nibble f = bus_fetch_nibble( pc );
    pc++;
    Nibble t = bus_fetch_nibble( pc );
    pc++;
